$(document).ready(function(){
			...
			warArray.forEach(function(item){
				if(yearQ>=item[3] && yearQ<=item[4]){
					getWikiImg(item[1], function(imgURL){
						if(imgURL!=""){
							$('#war_'+item[0]+' img').attr('src',imgURL);
						}
					});
				}
			});
	...
	function warRenderList(item){		
		...
		$("#load_space").append('<li class="media border-bottom pb-4 mb-4" id="war_'+item[0]+'">');
		$("#load_space #war_"+item[0]).append('<img width="220px" class="align-self-start mr-3" src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/80/Flag_of_the_United_States_%281877%E2%80%931890%29.svg/180px-Flag_of_the_United_States_%281877%E2%80%931890%29.svg.png">');
		$("#load_space #war_"+item[0]).append('<div class="media-body">');
		$("#load_space #war_"+item[0]+' .media-body').append('<h2 class="mt-0 mb-1">'+item[1]+'</h2>');
		$("#load_space #war_"+item[0]+' .media-body').append('<p class="lead">'+item[3]+'-'+item[4]+'</p>');
		$("#load_space #war_"+item[0]+" .media-body").append('<p class="">'+item[2]+'</p>');
		$("#load_space #war_"+item[0]+" .media-body").append('<a rel="ext" class="btn btn-primary" href="'+item[5]+'">Find out more on Wikipedia</a>');
		//$("#load_space").append('<br />');
	}
	function warRenderCard(item){		
		$("#load_space").append('<div class="col-sm-6 col-md-4" id="warCol_'+item[0]+'">');
		$("#load_space #warCol_"+item[0]).append('<div class="card" id="war_'+item[0]+'">');
		$("#load_space #war_"+item[0]).append('<img height="300px" class="card-img-top" src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/80/Flag_of_the_United_States_%281877%E2%80%931890%29.svg/100px-Flag_of_the_United_States_%281877%E2%80%931890%29.svg.png">');
		$("#load_space #war_"+item[0]).append('<h2 class="card-header">'+item[1]+'</h2>');
		$("#load_space #war_"+item[0]).append('<div class="card-body"><h5 class="card-title">'+item[3]+'-'+item[4]+'</h5>');
		$("#load_space #war_"+item[0]+" .card-body").append('<p class="card-text">'+item[2]+'</p>');
		$("#load_space #war_"+item[0]+" .card-body").append('<a rel="ext" class="btn btn-primary" href="'+item[5]+'">Find out more on Wikipedia</a>');
